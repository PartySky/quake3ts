/*
 * q3bspObj_worker.js - Parses Quake 3 Maps (.bsp) for use in WebGL
...
// Important, this q3bspObj is no q3bspObj from ./q3bspObj!
// import { q3bspObj } from "./q3bspObj";
...
export var q3bspObj = {
    ...
    readLeaves: null,
    readLeafFaces: null,
    readLeafBrushes: null,
    readBrushes: null,
    readBrushSides: null,
    readVisData: null,
    brightnessAdjust: null,
    brightnessAdjustVertex: null,
    colorToVec: null,
    tesselate: null,
    getCurvePoint2: null,
    getCurvePoint3: null,
...
q3bspObj.parse = function (src, tesselationLevel) {
    ...
    // start dub call to q3bsp's function
    let msg = {
        data: {
            type: 'status',
            message: 'Map downloaded, parsing level geometry...'
    ...
    // Read map entities
    q3bspObj.readEntities(header.lumps[0], src);

    // Load visual map components
    shaders = q3bspObj.readShaders(header.lumps[1], src);
    var lightmaps = q3bspObj.readLightmaps(header.lumps[14], src);
    var verts = q3bspObj.readVerts(header.lumps[10], src);
    var meshVerts = q3bspObj.readMeshVerts(header.lumps[11], src);
    faces = q3bspObj.readFaces(header.lumps[13], src);

    q3bspObj.compileMap(verts, faces, meshVerts, lightmaps, shaders, tesselationLevel);
    ...
    // Load bsp components
    planes = q3bspObj.readPlanes(header.lumps[2], src);
    nodes = q3bspObj.readNodes(header.lumps[3], src);
    leaves = q3bspObj.readLeaves(header.lumps[4], src);
    leafFaces = q3bspObj.readLeafFaces(header.lumps[5], src);
    leafBrushes = q3bspObj.readLeafBrushes(header.lumps[6], src);
    brushes = q3bspObj.readBrushes(header.lumps[8], src);
    brushSides = q3bspObj.readBrushSides(header.lumps[9], src);
    var visData = q3bspObj.readVisData(header.lumps[16], src);
...
q3bspObj.readLightmaps = function(lump, src) {
    ...
    for (var i = 0; i < count; ++i) {
        ...
        for (var j = 0; j < lightmapSize * 4; j += 4) {
            ...
            q3bspObj.brightnessAdjust(rgb, 4.0);
...
q3bspObj.readVerts = function(lump, src) {
    ...
    for (var i = 0; i < count; ++i) {
        elements.push({
            ...
            color: q3bspObj.brightnessAdjustVertex(q3bspObj.colorToVec(src.readULong()), 4.0)
        });
    }
    ...
    for (var i = 0; i < faces.length; ++i) {
        ...
        if (face.type == 1 || face.type == 2 || face.type == 3) {
            ...
            if (face.type == 1 || face.type == 3) {
                ...
                // Build Bezier curve
                q3bspObj.tesselate(face, verts, meshVerts, tesselationLevel);
...
q3bspObj.tesselate = function(face, verts, meshVerts, level) {
    ...
    for (var py = 0; py < face.size[1] - 2; py += 2) {
        for (var px = 0; px < face.size[0] - 2; px += 2) {
            ...
            var c0 = verts[off + rowOff + px],
                c1 = verts[off + rowOff + px + 1],
                c2 = verts[off + rowOff + px + 2];
            rowOff += face.size[0];
            var c3 = verts[off + rowOff + px],
                c4 = verts[off + rowOff + px + 1],
                c5 = verts[off + rowOff + px + 2];
            rowOff += face.size[0];
            var c6 = verts[off + rowOff + px],
                c7 = verts[off + rowOff + px + 1],
                c8 = verts[off + rowOff + px + 2];
            ...
            for (var i = 0; i < L1; ++i) {
                ...
                var pos = q3bspObj.getCurvePoint3(c0.pos, c3.pos, c6.pos, a);
                var lmCoord = q3bspObj.getCurvePoint2(c0.lmCoord, c3.lmCoord, c6.lmCoord, a);
                var texCoord = q3bspObj.getCurvePoint2(c0.texCoord, c3.texCoord, c6.texCoord, a);
                var color = q3bspObj.getCurvePoint3(c0.color, c3.color, c6.color, a);
            ...
            for (var i = 1; i < L1; i++) {
                ...
                var pc0 = q3bspObj.getCurvePoint3(c0.pos, c1.pos, c2.pos, a);
                var pc1 = q3bspObj.getCurvePoint3(c3.pos, c4.pos, c5.pos, a);
                var pc2 = q3bspObj.getCurvePoint3(c6.pos, c7.pos, c8.pos, a);

                var tc0 = q3bspObj.getCurvePoint3(c0.texCoord, c1.texCoord, c2.texCoord, a);
                var tc1 = q3bspObj.getCurvePoint3(c3.texCoord, c4.texCoord, c5.texCoord, a);
                var tc2 = q3bspObj.getCurvePoint3(c6.texCoord, c7.texCoord, c8.texCoord, a);

                var lc0 = q3bspObj.getCurvePoint3(c0.lmCoord, c1.lmCoord, c2.lmCoord, a);
                var lc1 = q3bspObj.getCurvePoint3(c3.lmCoord, c4.lmCoord, c5.lmCoord, a);
                var lc2 = q3bspObj.getCurvePoint3(c6.lmCoord, c7.lmCoord, c8.lmCoord, a);

                var cc0 = q3bspObj.getCurvePoint3(c0.color, c1.color, c2.color, a);
                var cc1 = q3bspObj.getCurvePoint3(c3.color, c4.color, c5.color, a);
                var cc2 = q3bspObj.getCurvePoint3(c6.color, c7.color, c8.color, a);

                for (let j = 0; j < L1; j++) {
                    var b = j / level;

                    var pos = q3bspObj.getCurvePoint3(pc0, pc1, pc2, b);
                    var texCoord = q3bspObj.getCurvePoint2(tc0, tc1, tc2, b);
                    var lmCoord = q3bspObj.getCurvePoint2(lc0, lc1, lc2, b);
                    var color = q3bspObj.getCurvePoint3(cc0, cc1, cc2, a);
...
q3bspObj.trace = function(traceId, start, end, radius, slide) {
    ...
    q3bspObj.traceNode(0, 0, 1, start, end, radius, output);
...
q3bspObj.traceNode = function(nodeIdx, startFraction, endFraction, start, end, radius, output) {
    if (nodeIdx < 0) { // Leaf node?
        ...
        for (var i = 0; i < leaf.leafBrushCount; i++) {
            ...
            if (brush.brushSideCount > 0 && (shader.contents & 1)) {
                q3bspObj.traceBrush(brush, start, end, radius, output);
            }
        }
        return;
    }
    ...
    if (startDist >= radius && endDist >= radius) {
        q3bspObj.traceNode(node.children[0], startFraction, endFraction, start, end, radius, output);
    } else if (startDist < -radius && endDist < -radius) {
        q3bspObj.traceNode(node.children[1], startFraction, endFraction, start, end, radius, output);
    } else {
        var side;
        ...
        q3bspObj.traceNode(node.children[side], startFraction, middleFraction, start, middle, radius, output);
        ...
        q3bspObj.traceNode(node.children[side === 0 ? 1 : 0], middleFraction, endFraction, middle, end, radius, output);
    }
};
    ...
    for (var i = 0; i < leaves.length; ++i) {
        ...
        if (q3bspObj.checkVis(curLeaf.cluster, leaf.cluster)) {
            for (var j = 0; j < leaf.leafFaceCount; ++j) {
                ...
                // HOTFIX - why 2 brackets are used? [[]] 
                var face = faces[leafFaces[j + leaf.leafFace]];
                if (face) {
