...
function drawFrame(gl) {
    ...
    if (!isVRPresenting()) {
        ...
        map.draw(leftViewMat, vrFrameData.leftProjectionMatrix, leftViewport, rightViewMat, vrFrameData.rightProjectionMatrix, rightViewport);
    }
    else {
        var canvas = document.getElementById("viewport");
        var leftEye = vrDisplay.getEyeParameters("left");
...
function updateInput(frameTime) {
    ...
    for (var i = 0; i < gamepads.length; ++i) {
        ...
        if (pad) {
            ...
            moveLookLocked(filterDeadzone(pad.axes[2]) * 25.0, filterDeadzone(pad.axes[3]) * 25.0);
